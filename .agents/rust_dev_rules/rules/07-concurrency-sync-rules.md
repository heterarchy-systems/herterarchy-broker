...
- bounded application queue가 full일 때 producer thread를 blocking `send`로 무기한 누적시키지 않는다. pre-application saturation은 fail-fast reject하고 queue depth는 configured capacity를 넘지 않아야 한다.
...
- bounded Raft RPC connection queue는 worker/queue limit까지 실제 포화시킨 뒤 초과 connection이 reject되고 load가 다시 drain되는지 검증한다.
- application state-owner queue도 observable active/queued barrier로 실제 포화시켜 반복 초과 요청의 fail-fast rejection, bounded depth 불변, drain 후 정상 progress를 검증한다.
- bounded client connection churn은 실제 connect/request/close를 반복한 뒤 worker/state-owner drain과 cluster quorum progress를 검증한다.
