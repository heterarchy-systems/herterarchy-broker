...
- `agent-broker/`는 ChatToCodex에서 독립적으로 빌드·테스트·실행 가능한 Agent Broker 경계다.
- `agent-broker/`의 build/test/run은 ChatToCodex Python package, browser runtime, control panel, 또는 ChatToCodex repository-local state에 의존하지 않는다.
...
- 기존 Python Broker는 Rust migration 동안 reference implementation/conformance oracle로 취급한다.
- Rust 구현이 parity를 증명하기 전 기존 Python 경로를 성급하게 삭제하지 않는다.
